...
<body class="h-screen flex">
    <div class="container flex w-full">
        ...
        <div class="w-1/2 bg-[#485A48] text-white flex flex-col justify-center px-16 relative">
            ...
            <h2 class="title text-2xl font-bold text-center mb-8 relative z-10">
                {{ request('role') ? strtoupper(request('role')) . ' LOGIN' : 'USER LOGIN' }}
            </h2>
            ...
            <form method="POST" action="{{ route('login') }}" class="login-form w-full relative z-10">
                ...
                <div class="input-group relative mb-5">
                    ...
                    <input type="text" name="username_or_email" placeholder="Email or Username" value="{{ old('username_or_email') }}" required 
                        class="w-full pl-12 pr-4 py-3.5 rounded-lg focus:outline-none {{ $errors->has('username_or_email') ? 'border-2 border-red-500 text-red-500 placeholder-red-400' : 'text-gray-700' }}">
                ...
                <div class="input-group relative mb-5">
                    ...
                    <input type="password" name="password" placeholder="Password" required 
                        class="w-full pl-12 pr-4 py-3.5 rounded-lg focus:outline-none {{ $errors->has('password') || $errors->has('username_or_email') ? 'border-2 border-red-500 text-red-500 placeholder-red-400' : 'text-gray-700' }}">
                    @if($errors->has('password'))
                        <span class="absolute right-4 top-1/2 transform -translate-y-1/2 text-red-500 text-xs font-bold">{{ $errors->first('password') }}</span>
                    @elseif($errors->has('username_or_email'))
                        <span class="absolute right-4 top-1/2 transform -translate-y-1/2 text-red-500 text-xs font-bold">Invalid!</span>
                    @endif
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const inputs = document.querySelectorAll('input[name="username_or_email"], input[name="password"]');
